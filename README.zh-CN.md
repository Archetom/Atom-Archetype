...
本文档对应尚未发布到 Maven Central 的 **2.0.0-SNAPSHOT**。Maven Central 上的 `1.1.0` 是 Spring Boot 3.5 旧架构，不包含本文描述的安全、租户、Flyway 与命令/查询模板改造。
...
当前生成命令固定使用本地安装的 `2.0.0-SNAPSHOT`。正式发布 `2.0.0` 后，应改为精确的发布版本以保证可复现。Archetype 升级不会自动改写已经生成的项目，请参考 [升级指南](docs/upgrade-guide.md)。
