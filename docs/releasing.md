...
Atom Archetype publishes through the Sonatype Central Portal. `make deploy` uploads and waits for Central validation; public release remains an explicit Portal action.
...
The target refuses SNAPSHOT versions and dirty worktrees. After the command reports that validation succeeded:
